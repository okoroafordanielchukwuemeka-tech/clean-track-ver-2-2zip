...
export default function OrderDetail() {
  ...
  const isAssignedWorker = user?.type !== "worker" || order.assignedWorkerId === user?.id;
  const canClaimOrder =
    user?.type === "worker" &&
    !order.assignedWorkerId &&
    (hasPermission("canRecordPickups") || hasPermission("canProcessOrders"));
  const canVerifyOrder =
    isAssignedWorker &&
    !order.isVerified &&
    ["pending", "processing"].includes(order.status) &&
    (isOwner || hasPermission("canRecordPickups") || hasPermission("canProcessOrders"));
      ...
      {!isCancelled && (
        <Card>
          <CardContent className="p-4">
            ...
            {/* Worker controls: no branch routing, just claim and operate the order. */}
            {(canClaimOrder || canVerifyOrder) && (
              <div className="flex items-center gap-2 mb-3">
                {canClaimOrder && (
                  <Button
                    variant="outline"
                    onClick={() => updateMutation.mutate({ assignedWorkerId: user?.id })}
                    disabled={updateMutation.isPending}
                  >
                    {updateMutation.isPending ? "Claiming…" : "Claim Order"}
                  </Button>
                )}
                {canVerifyOrder && (
                  <Button
                    ...
                    disabled={updateMutation.isPending}
                  >
                    <CheckCircle className="h-4 w-4 mr-1" />Verify Clothes
                  </Button>
                )}
              </div>
            )}
